...
**Текущее состояние:** приложение запускается, показывает обои за иконками рабочего стола. PAT, список репозиториев, интервалы опроса, автозапуск и автопауза настраиваются в окне «Настройки»; конфигурация сохраняется в `settings.json`.
