...
**Текущее состояние:** приложение запускается, показывает обои за иконками рабочего стола и карточку `microsoft/vscode` (репозиторий по умолчанию). PAT настраивается в окне «Настройки»; список репозиториев и сохранение конфигурации — на этапе 3.
