...
**Текущее состояние:** на обоях отображаются карточки репозиториев с PR, issues, релизами, CI badge, heatmap и лентой событий. PAT, список репозиториев, интервалы опроса, автозапуск и автопауза — в окне «Настройки»; конфигурация в `settings.json`.
