...
| 3 | [Мульти-репо и настройки](#этап-3--мульти-репо-и-настройки) | 🔄 В работе | Список репо, токен, автозапуск |
...
**Текущее состояние:** приложение запускается, показывает обои за иконками рабочего стола. PAT и список репозиториев настраиваются в окне «Настройки»; список сохраняется в `settings.json`. По умолчанию — `microsoft/vscode`.
...
- [ ] Настройки сохраняются между перезапусками (список репо — да; интервалы и автозапуск — нет)
