...
1. Скачайте `GitHubWallpaper.exe` из [Releases](https://github.com/rkfsociety/GitHubWallpaper/releases/latest) — появляется автоматически после каждого push в `main`.
2. Положите файл в любую папку (установка не требуется).
3. Запустите двойным щелчком — приложение свернётся в трей, обои появятся за иконками рабочего стола.
...
Версия в exe задаётся автоматически в CI: `1.0.<номер сборки>` при push в `main` + автотег и Release. Локально без `-p:Version` — `1.0.0-dev`.
