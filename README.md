...
1. Скачайте `GitHubWallpaper.exe` из [Releases](https://github.com/rkfsociety/GitHubWallpaper/releases/latest).
...
Версия в exe задаётся автоматически в CI: `1.0.<номер сборки>` при push в `main`, из тега `v*` при релизе (например `v1.0.0` → `1.0.0`). Локально без `-p:Version` — `1.0.0-dev`.
